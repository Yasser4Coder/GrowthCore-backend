...
export const salesOverTimeHistogram = async (req, res) => {
  const { csvId } = req.params;
  ...
  try {
    const response = await axios.get(
      `https://dae0-41-111-224-162.ngrok-free.app/sales_over_time?_id=${csvId}`,
      {
        timeout: 10000, // timeout 10 second
      }
    );
    ...
    return res.status(200).json({
      message: "Sales over time histogram fetched successfully",
      data: response.data,
    });
  } catch (error) {
    if (error.response) {
